...
export function CrmProvider({ children }) {
  ...
  const connectGoogleLive = async () => {
    try {
      ...
      return true
    } catch (e) { toast(`Google sign-in failed: ${e.message}`, 'warn'); return false }
  }
  ...
  const connectMailbox = async provider => {
    if (provider === 'gmail' && googleClientId) {
      try {
        ...
        toast(`Gmail live failed: ${e.message} — enable the Gmail API in your Google Cloud project`, 'warn')
        return false
      }
    }
