  ...
  return (
    <Card className="overflow-hidden">
      <div role="button" tabIndex={0}
        className="w-full flex items-center gap-3 p-4 text-left flex-wrap hover:bg-[var(--hover)] transition-colors cursor-pointer select-none"
        onClick={onToggle} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onToggle() } }}>
        <span className="w-9 h-9 rounded-xl grid place-items-center flex-none" style={{ background: '#818cf81c', color: 'var(--t-indigo)' }}><FileUp size={16} /></span>
      ...
      {open && (
        <div className="border-t fadein" style={{ borderColor: 'var(--hairline)' }}>
          {!b.details && (
          ...
          {b.details && b.details.map((d, i) => {
            ...
            return (
              <div key={i} className="flex items-start gap-3 px-4 py-3 border-b last:border-0" style={{ borderColor: 'var(--hairline)', opacity: b.rolledBack ? .5 : 1 }}>
                {contact ? <Avatar name={d.name} size={30} /> : <span className="w-[30px] h-[30px] rounded-full grid place-items-center flex-none text-[11px] font-bold" style={{ background: 'var(--chipbg)', color: 'var(--faint)' }}>?</span>}
                <div className="min-w-0 flex-1">
                        ...
                        <div key={j} className="flex items-center gap-2 text-[12px] flex-wrap font-mono">
                          ...
                          {ch.field !== 'record' && (
                            <>
                              ...
                              <span style={{ color: 'var(--t-green)' }}>{ch.to}</span>
                            </>
                          )}
                          {ch.field === 'record' && <span style={{ color: 'var(--t-green)' }}>created as new contact</span>}
                        </div>
                      ))}
                    </div>
                  )}
  ...
  return (
    <Modal open onClose={onClose} title={`Rollback “${batch.source}”?`}>
      ...
      <ul className="text-[13px] flex flex-col gap-1.5 mb-4" style={{ color: 'var(--muted)' }}>
        <li>• <strong style={{ color: 'var(--t-rose)' }}>{dels} contact(s)</strong> created by this import will be removed</li>
        <li>• <strong style={{ color: 'var(--t-amber)' }}>{reverts} contact(s)</strong> will be restored to their pre-import values</li>
...
function SyncRuns() {
  ...
  return (
    <Card className="p-4">
      ...
      {runs.map(a => (
        <div key={a.id} className="flex items-center gap-3 py-3 border-b last:border-0 flex-wrap" style={{ borderColor: 'var(--hairline)' }}>
