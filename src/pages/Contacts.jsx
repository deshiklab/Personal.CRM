...
import { Search, UserPlus, Star, Phone, Mail, Cake, Users, Clock, CheckCircle2, StickyNote, X, Heart, Gift, Globe, MessageCircle, Link2, Share2 } from 'lucide-react'
...
export default function Contacts() {
  ...
  const list = useMemo(() => {
    let xs = contacts.filter(c => {
      ...
      if (groupF && c.groupId !== groupF) return false
    ...
    <div className="max-w-[1200px] mx-auto">
      <SectionHead kicker="Directory" title="Contacts"
        ...
        right={<div className="flex items-center gap-2">
          <CsvButton filename="contacts.csv" rows={crm.contacts} headers={[
            ...
            { label: 'Group', get: r => crm.groupById[r.groupId]?.name || '' },
      ...
      <div className="card overflow-hidden overflow-x-auto">
        <table className="table min-w-[720px]">
          <thead><tr><th style={{ width: 34 }}></th><th>Name</th><th>Group</th><th>Tags</th><th>Follow-up</th><th>Last contact</th></tr></thead>
          <tbody>
            {list.map(c => {
              ...
              return (
                <tr key={c.id} className="rowclick" onClick={() => setOpenId(c.id)}>
                  ...
                  <td>{g && <Pill color={g.color}>{g.name}</Pill>}</td>
                  <td>
...
function ContactDrawer({ contact: c, onOpen, onClose }) {
  const crm = useCrm()
  ...
  const g = crm.groupById[c.groupId]
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex gap-1.5 mt-2 flex-wrap">
            {g && <Pill color={g.color}>{g.name}</Pill>}
...
function AddContactModal({ open, onClose }) {
  const { addContact, groups, relFreq, tags, toast, contacts } = useCrm()
  const empty = { name: '', role: '', company: '', phone: '', email: '', birthday: '', anniversary: '', groupId: 'g_leads', rel: 'acquaintance', tags: [], introducedBy: '', interests: '' }
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add contact" wide>
      ...
      <div className="flex justify-end gap-2 mt-6">
        ...
        <button className="btn btn-primary" disabled={!f.name.trim()}
          onClick={() => {
            ...
            toast(`Added ${f.name.trim().split(' ')[0]} to contacts`); onClose()
          }}>
